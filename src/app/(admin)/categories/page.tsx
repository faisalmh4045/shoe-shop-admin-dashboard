...
import { getCategories } from "@/dal/categories.dal";
...
export default async function CategoriesPage({
  ...
  const { data, count } = await getCategories(page, PAGE_SIZE);
      ...
      {count === 0 ? (
        <div className="flex flex-col items-center justify-center gap-4 rounded-lg border border-dashed border-border bg-muted/30 px-6 py-16 text-center">
          <p className="text-sm text-muted-foreground">
            No categories yet. Create your first category to see it here.
          </p>
          <Button asChild>
            <Link href="/categories/new">
              <Plus data-icon="inline-start" />
